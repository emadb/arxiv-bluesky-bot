...
pub struct Config {
    ...
    /// Which announce types to post (lowercased, canonical form). Default: only
    /// brand-new papers. Empty means "any type".
    ...
    pub dry_run: bool,
    /// Timezone arXiv stamps its announcements in (parsed once, at the boundary,
    /// so an invalid value fails fast instead of deep in the pipeline).
    pub time_zone: Tz,
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        let tz_name = opt("ARXIV_TIMEZONE").unwrap_or_else(|| "America/New_York".to_string());
        let time_zone: Tz = tz_name
            .parse()
            .map_err(|_| anyhow!("invalid ARXIV_TIMEZONE: {tz_name}"))?;
